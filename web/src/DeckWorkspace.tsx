  ...
  return <main className="min-h-screen bg-background text-foreground">
    ...
    <div className="mx-auto max-w-[1540px] px-5 py-7 sm:px-8 lg:py-10">
      ...
      <section>
        ...
        <div className="space-y-8">{groups.map(([label, entries]) => <section key={label}><div className="mb-3 flex items-center gap-3"><h3 className="font-heading text-lg font-semibold text-zinc-200">{label}</h3><span className="text-xs text-zinc-600">{entries.reduce((sum, card) => sum + card.quantity, 0)} cards</span><span className="h-px flex-1 bg-white/8" /></div>{view === 'art' ? <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-7">{entries.map((card) => <article key={card.key ?? card.name} className="group relative overflow-hidden rounded-xl border border-white/8 bg-[#101311] shadow-xl shadow-black/10"><div className="aspect-[63/88] overflow-hidden bg-white/[0.025]"><CardArtwork card={card} /></div><span className="absolute left-2 top-2 rounded-md bg-black/80 px-2 py-1 font-mono text-xs text-white">{card.quantity}×</span>{commander === card.name && <span className="absolute right-2 top-2 grid size-7 place-items-center rounded-full bg-lime-300 text-[#10140c]"><Crown className="size-3.5" /></span>}<div className="p-3"><p className="truncate text-xs font-medium text-zinc-200">{card.name}<FoilIndicator cacheKey={card.cardData?.cacheKey} /></p><p className="mt-1 truncate text-[10px] text-zinc-600">{card.cardData?.setCode ? `${card.cardData.setCode} #${card.cardData.collectorNumber}` : card.cardData?.typeLine ?? 'Card data unavailable'}{cardPrice(card) ? ` · $${cardPrice(card)}` : ''}</p>{card.cardData?.artist && <p className="mt-1 truncate text-[9px] text-zinc-700">Art by {card.cardData.artist}</p>}</div></article>)}</div> : <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{entries.map((card) => <article key={card.key ?? card.name} className="flex items-center gap-3 rounded-xl border border-white/8 bg-[#101311] px-4 py-3"><span className="font-mono text-sm text-zinc-500">{card.quantity}×</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-zinc-200">{card.name}<FoilIndicator cacheKey={card.cardData?.cacheKey} />{commander === card.name && <Crown className="ml-2 inline size-3.5 text-lime-300" />}</p><p className="truncate text-[10px] text-zinc-600">{card.cardData?.setCode ? `${card.cardData.setCode} #${card.cardData.collectorNumber}` : card.cardData?.typeLine ?? 'Card data unavailable'} · {titleCase(card.cardData?.rarity ?? '')}</p></div><div className="text-right"><p className="font-mono text-xs text-zinc-400">{cardPrice(card) ? `$${cardPrice(card)}` : '—'}</p><p className="text-[10px] text-zinc-600">MV {card.cardData?.manaValue ?? '—'}</p></div></article>)}</div>}</section>)}</div>
